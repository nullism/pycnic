...
def nostdout():
    ...
    # Store the original stdout and stderr to restore later
...
def alpha_sort(routes, verbose=False):
    ...
    for route in routes:
        ...
        print(ljust_route + '  ' + ljust_methods + '  ' + full_class_name(route[2]))
        if verbose:
...
def no_sort(routes, verbose=False):
    """Don't process the list at all, simply print it in a digestible format."""
    max_route_length = 5 # Length of the word "route"
    max_method_length = 6 # Length of the word "method"
    # Determine justified string lengths
    for route in routes:
...
def method_sort(routes, verbose=False):
    ...
    # Find which routes support which methods, and place into categories accordingly
    ...
    for key in method_routes:
        if len(method_routes[key]) > 0:
            ...
            for a in method_routes[key]:
                ...
                print(ljust_method + '  ' + ljust_route + '  ' + full_class_name(a[1]))
                if verbose:
...
def find_routes():
    ...
    for route_regex, route_class in wsgi_class.routes:
        # Old-style python2 classes require you to call __class__ before __name__.
        ...
        class_http_methods = [method for method in http_methods if method in dir(route_class)]
